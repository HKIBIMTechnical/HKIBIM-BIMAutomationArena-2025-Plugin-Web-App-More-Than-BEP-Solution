...
import pandas as pd
import requests
# st.write("### Model Element Data")


file_link = r"files/googleDrive_ModelElement_Files_link.xlsx"
...
with col1:
    ...
    with cola:
        selected_NE_file = st.selectbox("Model Element - NE Data - Select NE file", df["name"][df["name"].str.contains("-NE-")])
...
with col2:
    cola,colb = st.columns([2,1], vertical_alignment="bottom",gap="small")
    with cola:
        selected_EX_file = st.selectbox("Model Element - EX Data - Select EX file", df["name"][df["name"].str.contains("-EX-")])
